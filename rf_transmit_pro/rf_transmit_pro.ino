...
void loop() {
  ...
  if(transmittedFlag) {
    ...
    // 失败
    // 添加
    // byte byteArr[] = {0xFF,0x55,0x55,0x55,0x55,0x54,0x21,0xA4,0x23,    0x03,0x0E,   0xAB,0xCD,0xEF,    0x60,    0x73,0x75,    0x9B,0x00,   0x36,0xAF,0x6C,   0x01}; 
    // int state = radio.startTransmit(byteArr, 23);
    // 开
    // byte byteArr[] = {0xFF,0x55,0x55,0x55,0x55,0x54,0x21,0xA4,0x23,  0x03,0x10,   0xAB,0xCD,0xEF,    0xE7,    0xC9,0x4D,    0x9B,0x00,   0x36,0xAF,0x6C,   0x04,0x02,   0x01}; 
    // int state = radio.startTransmit(byteArr, 25);
  
    // 失败
    // 添加
    // byte byteArr[] = {0xFF,0x55,0x55,0x55,0x55,0x54,0x21,0xA4,0x23,    0x03,0x0E,   0x3A,0x96,0x9D,    0x60,    0xB0,0x1D,    0xAB,0xCD,   0x36,0xAF,0x6C,   0x01}; 
    // int state = radio.startTransmit(byteArr, 23);
    // 开
    // byte byteArr[] = {0xFF,0x55,0x55,0x55,0x55,0x54,0x21,0xA4,0x23,  0x03,0x10,   0x3A,0x96,0x9D,    0xE7,    0x67,0xE4,    0xAB,0xCD,   0x36,0xAF,0x6C,   0x04,0x02,   0x01}; 
    // int state = radio.startTransmit(byteArr, 25);

    // 失败
    // 添加
    // byte byteArr[] = {0xFF,0x55,0x55,0x55,0x55,0x54,0x21,0xA4,0x23,    0x03,0x0E,   0x3A,0x96,0x9D,    0x60,    0xC9,0x8A,    0x9B,0x00,   0xAB,0xCD,0xEF,   0x01}; 
    // int state = radio.startTransmit(byteArr, 23);
    // 开
    // byte byteArr[] = {0xFF,0x55,0x55,0x55,0x55,0x54,0x21,0xA4,0x23,  0x03,0x10,   0x3A,0x96,0x9D,    0xE7,    0x26,0x7B,    0x9B,0x00,   0xAB,0xCD,0xEF,   0x04,0x02,   0x01}; 
    // int state = radio.startTransmit(byteArr, 25);
  }
}